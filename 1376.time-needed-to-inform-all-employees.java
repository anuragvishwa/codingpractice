...
class Solution {
    public int numOfMinutes(int n, int headID, int[] manager, int[] informTime) {
		...
		return minutes(map,informTime,headID);	    
    }
    
     int minutes(Map<Integer,ArrayList<Integer>> map,int[] informTime,int headID){
		int max = 0;
		...
		for(int i=0;i<map.get(headID).size();i++){
			max = Math.max(max,minutes(map,informTime,map.get(headID).get(i)));
		}
